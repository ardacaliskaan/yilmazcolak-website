// data/teamData.js - Gerçek Bilgilerle Admin Panel Ready Team Structure
...
// Specialization Areas for Admin Panel
...
export const getTeamMembersByPosition = (position) => {
  return teamData.filter(member => member.position === position && member.isActive);
};

export const getFeaturedTeamMembers = () => {
  return teamData.filter(member => member.featuredOnHomepage && member.isActive)
                 .sort((a, b) => a.sortOrder - b.sortOrder);
};

export const getTeamMemberBySlug = (slug) => {
  return teamData.find(member => member.slug === slug && member.isActive);
};